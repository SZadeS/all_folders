...
describe 'Create tests for a hash' do
  ...
  it 'each fruit should have 4 keys' do
    ...
    expect(small_fruit_nutrition_details[:avocado].keys.length).to eql(4)
    expect(small_fruit_nutrition_details[:kiwifruit].keys.length).to eql(4)
    expect(small_fruit_nutrition_details[:plums].keys.length).to eql(4)
  end
